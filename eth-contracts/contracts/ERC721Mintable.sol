...
contract Pausable is Ownable {
    ...
    event Paused(address requester);
    event UnPaused(address requester);
    ...
    function setOperatingStatus(bool mode) public onlyOwner {
        require(_paused != mode, "Contract already in this mode");
        _paused = mode;
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(ownerOf(tokenId) != to, "Address is already of the token");

        // TODO DO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(isOwner() || isApprovedForAll(ownerOf(tokenId), msg.sender), "Only owner or an operator approved can to approve");
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(!to.isContract(), "TO Address is invalid");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "FROM should be a token owner");
...
contract ERC721MintableComplete is ERC721Metadata("Real State Token", "RST", "https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/") {
